...
$scope.moveWall = function(place) {
  if ($scope.activeWall === "") {
    ...
    if ($scope.mapCoors[place].drag == true) {
      document.getElementById("putWall").play();
    ...
    if ($scope.mapCoors[place].drop == true) {
      ...
      document.getElementById("dropWall").play();
    } else {
...
$scope.playerMove = function(event) {
  if ($scope.gameStarted === true) {
    ...
    if (event.keyCode == "13") {
      if ($scope.newPlayer.length) {
        $scope.addNewPlayer();
        $scope.newPlayer.focus();
...
$scope.timerFunc = function() {
  ...
  if ($scope.theDiceValue == 0) {
    ...
    $scope.infoMsg = 2;
    $scope.nextPlayer();
    document.getElementById("minTurn").play();
  }
  $scope.stepsHistory = [];
}
